...
@(ev '(require fraud-plus fraud-plus/compiler/compile fraud-plus/executor/exec))
...
@subsection[#:tag-prefix "a4-" #:style 'unnumbered]{From Extort+ to Fraud+}

Implement the @racket[abs], unary @racket[-], and @racket[not]
operations and the @racket[cond] and @racket[case] forms from
@seclink["Assignment 3"]{Assignment 3} by modifying
@tt{interpreter/interp.rkt}, @tt{interpreter/interp-prim.rkt},
@tt{compiler/compile.rkt}, and @tt{compiler/compile-ops.rkt}. You can
start from your previous code.
...
@tt{test/define-tests.rkt}.  There is function called @racket[test]
...
test/run-interp-tests.rkt} will test the interpreter and @tt{raco test
test/run-compile-tests.rkt} will test the compiler.  You are encouraged to add
your own tests.
