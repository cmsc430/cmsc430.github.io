...
When implementing a compiler, we have two choices to make before
getting started:

@itemlist[
@item{what is the language of programs that the compiler
will compile?}
@item{what is the language that the compiler will be written in?}
]
...
for (a subset of) it.  That language is Racket.
...
has these features suitable for writing compilers.  That language is
Racket.

So we have two reasons to learn Racket: we will need to know enough
Racket to write a compiler and we will need to know the parts of
Racket we plan to compile so that we build a correct compiler.
